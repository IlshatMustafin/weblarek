...
### Архитектурные события (Презентер)
- `card:select` — инициируется презентером при вызове переданного во View коллбэка `onClick` у карточки каталога. Начинает процесс открытия карточки.
- `card:toggle-basket` — инициируется презентером при вызове коллбэка кнопки действия карточки превью. Добавляет или удаляет товар.
- `basket:remove` — инициируется презентером при клике на кнопку удаления в строке корзины.
...
### Подход к реализации
В приложении используется классическая событийно-ориентированная архитектура MVP на базе единой точки входа. Презентер не содержит собственной логики хранения состояний. Взаимодействие со слоем отображения строится на основе инверсии зависимостей (Dependency Inversion): при создании компонентов представления (например, `CardCatalog`) в их конструкторы через объект `actions` внедряются анонимные функции-коллбэки. Это позволяет полностью изолировать компоненты View от моделей данных и глобального брокера событий.

### Принцип реактивного обновления (Событийная цепочка)
Презентер функционирует исключительно как обработчик событий. В нем строго соблюдается правило реактивности: вызов методов мутации данных в моделях никогда не приводит к синхронному вызову методов `render` у компонентов представления. Обновление интерфейса происходит строго в ответ на события, генерируемые моделями данных (`CatalogModel`, `CartModel`) при изменении их внутреннего состояния.

## События приложения

### События моделей данных (Model)
- `items:changed` — генерируется `CatalogModel` при получении новых товаров с сервера. Перерисовывает галерею на главной.
- `basket:changed` — генерируется `CartModel` при добавлении, удалении или очистке корзины. Перерисовывает счетчик в хедере и состав корзины.
- `buyer:form-errors` — генерируется `BuyerModel` при изменении данных и передает объект ошибок `TFormErrors` для отображения в UI.

### События компонентов представления (View)
- `basket:open` — клик по иконке корзины в хедере. Открывает модальное окно корзины.
- `order:open` — клик по кнопке «Оформить» в корзине. Открывает форму первого шага.
- `order:submit` — отправка формы первого шага (выбор оплаты и адрес). Переводит на форму контактов.
- `contacts:submit` — отправка формы второго шага (контакты). Запускает отправку заказа на сервер.
- `success:close` — клик по кнопке «За покупками» в финальном окне успешной оплаты.
- `modal:open` — активация модального окна. Блокирует скролл страницы.
- `modal:close` — закрытие модального окна. Восстанавливает скролл страницы.

- `order:address:change` — генерируется при вводе символов в инпут адреса. Передает `{ value: string }`.
- `order:payment-change` — клик по кнопке выбора оплаты. Передает `{ target: 'card' | 'cash' }`.
- `order:submit` — успешный клик по кнопке «Далее» на первом шаге. Начинает переход к контактам.
- `contacts:email:change` — ввод символов в инпут электронной почты. Передает `{ value: string }`.
- `contacts:phone:change` — ввод символов в инпут номера телефона. Передает `{ value: string }`.
- `contacts:submit` — успешный клик по кнопке «Оплатить» на втором шаге. Инициирует отправку данных на сервер.

- `order:open` — генерируется при клике на кнопку «Оформить» в корзине. Передает управление презентеру для открытия первого шага формы.

- `success:close` — инициируется презентером при вызове коллбэка `onClick` у кнопки закрытия окна успешной оплаты. Возвращает пользователя к покупкам и закрывает модальное окно.
